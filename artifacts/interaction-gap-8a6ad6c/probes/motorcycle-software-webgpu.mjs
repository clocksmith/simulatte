...
const b=await openBrowserAudit({publicRoot:root+'/public',viewport:{width:1440,height:1000},webgpu:true,linuxVulkan:false,args:['--no-sandbox','--disable-dev-shm-usage','--use-angle=swiftshader','--use-webgpu-adapter=swiftshader','--enable-unsafe-swiftshader']});
...
try {
 ...
 for(const width of [1440,390]){
  ...
  await wait(1500);row.steps.push({name:'initial',state:await snap()});await shot('motorcycle-'+width+'-initial');
  ...
  await click('header-advanced');await click('run');
