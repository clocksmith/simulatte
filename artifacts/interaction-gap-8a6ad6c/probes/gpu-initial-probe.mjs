...
await click('decisions-button');await wait(200);row.steps.push({name:'advanced-paused',state:await snap()});await shot('gpu-'+width+'-advanced');
...
await ev(`(()=>{const e=document.querySelector('[data-plugin-control="coolantFlowLpm"]');e.value='20';e.dispatchEvent(new Event('change',{bubbles:true}));})()`);await wait(1500);row.steps.push({name:'coolant-20',state:await snap()});
await click('start-button');await until(`document.body.dataset.journeyPhase==='completed'`);row.steps.push({name:'intervention-completed',state:await snap()});await shot('gpu-'+width+'-intervention');
