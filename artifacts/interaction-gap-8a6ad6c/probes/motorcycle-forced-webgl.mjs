...
const root='/home/clocksmith/deco/simulatte', out=root+'/artifacts/interaction-gap-8a6ad6c/forced-webgl';
...
await c.send('Page.enable');await c.send('Page.addScriptToEvaluateOnNewDocument',{source:"Object.defineProperty(navigator,'gpu',{value:undefined});"});await c.send('Runtime.enable');await c.send('Network.enable');
...
try {
 ...
 for(const width of [1440,390]){
  ...
  console.log('comparison '+width);row.steps.push({name:'comparison',state:await snap(),result:await ev(`({before:document.getElementById('before').textContent,after:document.getElementById('after').textContent,visible:!document.getElementById('results').hidden})`)});await shot('motorcycle-'+width+'-comparison');
 }
