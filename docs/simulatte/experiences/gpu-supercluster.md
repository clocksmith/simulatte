...
2. Model hierarchical interconnects: intra-node $900\text{ GB/s}$ bidirectional NVLink (modeled as 450 GB/s per direction) and inter-node $800\text{ Gbps}$ InfiniBand.
...
5. Solve a facility heat-balance model, coolant flow delta-T, and die junction temperatures.
6. Apply a modeled uniform clock/power cap when demanded die temperatures exceed $80^\circ\text{C}$.
7. Model straggler tail-latency propagation across synchronous AllReduce collective barriers.


NVLink bandwidth reference: [NVIDIA Hopper tuning guide](https://docs.nvidia.com/cuda/archive/11.8.0/hopper-tuning-guide/index.html). The directional split is a model assumption; the configured `nvlinkBandwidthGbps` remains in gigabits per second.
...
| Coolant Flow | 120 L/min | 10 to 1000 L/min | Modulates fluid heat absorption capacity and loop temperatures |
...
- The thermal model solves heat balance from the unthrottled load estimate. Coldplate resistance scales from a fixed 120 L/min reference. A uniform linear power/clock cap holds the hottest modeled junction at 80 C, and the collective solver applies that cap to compute time. This two-pass scenario model does not model transient feedback or a calibrated hardware controller.
