...
| **2** | **256-GPU Supercluster (`/datacenter/gpu-supercluster-v1`)** | **Hero Datacenter Simulator**: Distributed AI Training | 3D liquid-cooled 42U rack arrays, NVLink mesh crossbars, Ring-AllReduce & Double-Binary Tree collectives, thermodynamic heat dissipation ODEs ($Q = \dot{m} C_p \Delta T$), and thermal throttling barriers. |
| **3** | **Sun Walker (`/city/sun-walker-v1`)** | **Hero City Simulator**: Local Urban Dynamics | Real-time solar azimuth calculations, building shadow projections, tree canopy occlusion, and shade-aware routing across NYC on a native WebGPU canvas. |
| **4** | **Maritime Trade (`/world/maritime-trade-global-v1`)** | **Hero Planetary Simulator**: Global Logistics & Cascades | Global shipping chokepoint simulation (Suez, Panama, Malacca), container port queues, disruption cascades, emissions tracking, and voyage counterfactuals. |
| **5** | **Orbital Transfer Planner (`/solar-system/orbital-transfer-planner-v1`)** | **Hero Orbital Simulator**: Interplanetary Missions | Grounded Lambert two-point boundary-value solver, Keplerian orbital mechanics, and $\Delta v$ porkchop plot trajectory optimization. |
| **6** | **Interstellar Relay (`/star-chart/interstellar-relay-network-v1`)** | **Hero Interstellar Simulator**: Relativistic Optical Comms | 3D Planck blackbody spectral stellar coronas (O, B, A, F, G, K, M), parsec galactic plane depth drop-lines, Gaussian laser beam divergence ($1.22 \lambda / D$), and moving-target light time. |
