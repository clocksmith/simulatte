(function attachSunExposure(root, factory) {
...
})(typeof globalThis !== 'undefined' ? globalThis : window, function createSunExposureModule() {
  ...
  function buildTimeVaryingEnvironmentField({
    ...
    const routeEvaluations = routes.map((route, routeIndex) => evaluateRoute({
      model,
  ...
  function finalizeEnvironmentField({ world, utcInstant, sampleSpacingM, initialSun, buildings, segmentRows, computeImplementation, timeSampling }) {
    ...
    validateEnvironmentField(field);
    return field;
  }
  ...
  function selectShadeAwareRoute({
    ...
    if (!Array.isArray(alternatives) || !alternatives.length) throw exposureError('shade_route_candidates_missing', 'Sun Walker expected route candidates from the routing SDK port');
  ...
  function defineCostModel({ id, version, fifo, evaluate, claimBoundary }) {
    if (!id || !version || typeof fifo !== 'boolean' || typeof evaluate !== 'function' || !claimBoundary) throw exposureError('shade_cost_model_invalid', id || 'missing');
    return Object.freeze({ schema: 'simulatte.timeDependentEdgeCostModel.v1', id, version, fifo, evaluate, claimBoundary });
  }

  function evaluateRoute({ model, segmentIds, worldModel, departureAt, routeCandidateId }) {
    const departureMs = Date.parse(departureAt);
    if (!Number.isFinite(departureMs)) throw exposureError('shade_departure_invalid', departureAt);
    ...
    const edgeRows = segmentIds.map((segmentId, edgeIndex) => {
      ...
      const result = model.evaluate({ segment: worldModel.segment(segmentId), segmentId, edgeIndex, enteredAt, arrivalOffsetSeconds, routeCandidateId });
      if (!Number.isFinite(result.traversalSeconds) || !Number.isFinite(result.generalizedCost)) throw exposureError('shade_edge_cost_invalid', segmentId);
      const row = {
        schema: 'simulatte.timeDependentEdgeEvaluation.v1', modelId: model.id, modelVersion: model.version,
        routeCandidateId, segmentId, edgeIndex, enteredAt, arrivalOffsetSeconds: round(arrivalOffsetSeconds),
        traversalSeconds: round(result.traversalSeconds), generalizedCost: round(result.generalizedCost),
        components: structuredClone(result.components || {}), evidence: structuredClone(result.evidence || {}),
      };
    ...
    return {
      schema: 'simulatte.timeDependentRouteEvaluation.v1', model: { schema: model.schema, id: model.id, version: model.version, fifo: model.fifo, claimBoundary: model.claimBoundary },
      routeCandidateId, departureAt: new Date(departureMs).toISOString(), arrivalAt: new Date(departureMs + arrivalOffsetSeconds * 1000).toISOString(),
      traversalSeconds: round(arrivalOffsetSeconds), generalizedCost: round(generalizedCost), edgeRows,
    };
  }

  function validateEnvironmentField(field) {
    if (!field || field.schema !== 'simulatte.environmentField.v1' || !Array.isArray(field.segmentRows)) {
      throw exposureError('environment_field_invalid', field?.schema || 'missing');
    }
    return field;
  }
