  ...
  function createContribution({ result, progressive }) {
    ...
    const layers = [
      ...result.stellarStates.map((state) => builder.layer({
        ...
        role: result.routeSelection.selectedPath.includes(state.sourceId) ? 'primary' : 'context',
        importance: result.routeSelection.selectedPath.includes(state.sourceId) ? 0.9 : 0.25,
        aggregationKey: result.routeSelection.selectedPath.includes(state.sourceId) ? null : 'stellar-neighborhood',
    ...
    const controls = builder.controls([
      ...
      numeric('maxHopDistancePc', 'Relays: maximum hop distance (pc)', result.controls.maxHopDistancePc, 0.01, 250000, 0.01, modeled),
      datetime('startEpochIso', 'Transmission: epoch (UTC)', result.controls.startEpochIso.slice(0, 16), modeled),
      numeric('packetBytes', 'Transmission: packet size (bytes)', result.controls.packetBytes, 64, 1073741824, 1, modeled),
      numeric('processingDelayHours', 'Transmission: relay processing delay (hours)', result.controls.processingDelayHours, 0, 8760, 1, modeled),
