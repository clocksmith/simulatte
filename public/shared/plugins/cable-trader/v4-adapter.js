(function attachCableTraderV4Adapter(root, factory) {
  ...
  const DATASET_REFERENCE = Object.freeze({
    ...
    sha256: '5b06038f025546f1dc3b5be7827925820e6a8ddbbdc5a8e72c92af419297627c',
    schemaId: 'simulatte.cableCompatibilityPriors.v1',
  });
  const CONFIG_HASH = '4ab8cc83c93eb0b35ff181c718367c4cfdf642f1921bc4cfc54bfa76deabcc94';
  ...
  function createContribution({ config, simulation, state, transferRoutes }) {
    const scenario = builder.datasetRecord(config.id, { sha256: CONFIG_HASH }, {
    ...
    const inspections = config.hubs.map((hub) => ({
      id: `inventory:${hub.id}`,
      label: `${hub.label} inventory`,
      targetIds: [`hub:${hub.id}`],
      fields: config.cableTypes.map((type) => ({
        id: type.id,
        label: type.label,
    ...
    return builder.contribution({
      ...
      inspections,
      provenanceRecords: [compatibility, scenario, ...hubRecords, eventModel, flowModel],
    });
  }
  ...
  return Object.freeze({ DATASET_REFERENCE, createContribution });
});
