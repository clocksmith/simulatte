// Fused Q4_K Matmul Kernel - WideTile register pattern (F16 A, Q4_K B, F16 C)
//
// W4A16 twin of fused_matmul_q4_widetile_f16.wgsl. The tiling and Q4_K
// dequantization path are identical, but the activation buffer is f16 so an
// execution-v1 f16 activation lane can keep prefill projections in f16 without
// an implicit widening cast.
...
var<workgroup> tileA: array<f16, MAX_TILE_M * QK_K>;

fn unpack_f16_lo(packed: u32) -> f16 { return f16(unpack2x16float(packed).x); }
fn unpack_f16_hi(packed: u32) -> f16 { return f16(unpack2x16float(packed).y); }
...
fn load_a_vec4(row_local: u32, base_elem: u32) -> vec4<f16> {
...
fn main(
    ...
    var results: array<f16, MAX_TILE_M>;
    ...
    for (var b: u32 = 0u; b < num_blocks; b = b + 1u) {
        ...
        if (col < u.N) {
            ...
            for (var p: u32 = 0u; p < NUM_SUBBLOCK_PAIRS; p = p + 1u) {
                ...
                let scale_even = d * f16(sm_even.x);
                let min_even_scalar = dmin * f16(sm_even.y);
                let scale_odd = d * f16(sm_odd.x);
                let min_odd_scalar = dmin * f16(sm_odd.y);
                let min_even = vec4<f16>(min_even_scalar);
                let min_odd = vec4<f16>(min_odd_scalar);
                ...
                for (var uidx: u32 = 0u; uidx < U32S_PER_PAIR; uidx = uidx + 1u) {
                    ...
                    let w_even = scale_even * vec4<f16>(lower_u32) - min_even;
                    let w_odd = scale_odd * vec4<f16>(upper_u32) - min_odd;
    ...
    if (col < u.N) {
        for (var m: u32 = 0u; m < TILE_M; m = m + 1u) {
            let row = row_base + m;
            if (row < u.M) {
                C[row * u.N + col] = results[m] * f16(u.alpha);
            }
        }
    }
}
