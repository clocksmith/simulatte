// scatter_add_dynamic_f16_weights_expert_scale.wgsl
...
@group(0) @binding(6) var<storage, read_write> output_dyn: array<f16>;
...
fn scatter_add_dynamic(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    for (var k: u32 = 0u; k < top_k; k = k + 1u) {
        ...
        let weight = routing_weights[routing_base + k] * f16(expert_scales[expert_idx]);
