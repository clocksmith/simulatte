// gather_split8_f16_vec4_f16_out.wgsl

/**
 * Gather Kernel (split8 F16 input -> F16 output, vec4)
 *
...
@group(0) @binding(10) var<storage, read_write> output_f16: array<f16>;
...
fn gather_vec4_f16_out(@builtin(global_invocation_id) gid: vec3<u32>) {
    ...
    if (token_id < section1_start) {
        ...
        output_f16[out_base] = embeddings0[embed_base];
        output_f16[out_base + 1u] = embeddings0[embed_base + 1u];
        output_f16[out_base + 2u] = embeddings0[embed_base + 2u];
        output_f16[out_base + 3u] = embeddings0[embed_base + 3u];
    } else if (token_id < section2_start) {
        local_token = token_id - section1_start;
        embed_base = local_token * u.input_hidden_size + dim_base;
        output_f16[out_base] = embeddings1[embed_base];
        output_f16[out_base + 1u] = embeddings1[embed_base + 1u];
        output_f16[out_base + 2u] = embeddings1[embed_base + 2u];
        output_f16[out_base + 3u] = embeddings1[embed_base + 3u];
    } else if (token_id < section3_start) {
        local_token = token_id - section2_start;
        embed_base = local_token * u.input_hidden_size + dim_base;
        output_f16[out_base] = embeddings2[embed_base];
        output_f16[out_base + 1u] = embeddings2[embed_base + 1u];
        output_f16[out_base + 2u] = embeddings2[embed_base + 2u];
        output_f16[out_base + 3u] = embeddings2[embed_base + 3u];
    } else if (token_id < section4_start) {
        local_token = token_id - section3_start;
        embed_base = local_token * u.input_hidden_size + dim_base;
        output_f16[out_base] = embeddings3[embed_base];
        output_f16[out_base + 1u] = embeddings3[embed_base + 1u];
        output_f16[out_base + 2u] = embeddings3[embed_base + 2u];
        output_f16[out_base + 3u] = embeddings3[embed_base + 3u];
    } else if (token_id < section5_start) {
        local_token = token_id - section4_start;
        embed_base = local_token * u.input_hidden_size + dim_base;
        output_f16[out_base] = embeddings4[embed_base];
        output_f16[out_base + 1u] = embeddings4[embed_base + 1u];
        output_f16[out_base + 2u] = embeddings4[embed_base + 2u];
        output_f16[out_base + 3u] = embeddings4[embed_base + 3u];
    } else if (token_id < section6_start) {
        local_token = token_id - section5_start;
        embed_base = local_token * u.input_hidden_size + dim_base;
        output_f16[out_base] = embeddings5[embed_base];
        output_f16[out_base + 1u] = embeddings5[embed_base + 1u];
        output_f16[out_base + 2u] = embeddings5[embed_base + 2u];
        output_f16[out_base + 3u] = embeddings5[embed_base + 3u];
    } else if (token_id < section7_start) {
        local_token = token_id - section6_start;
        embed_base = local_token * u.input_hidden_size + dim_base;
        output_f16[out_base] = embeddings6[embed_base];
        output_f16[out_base + 1u] = embeddings6[embed_base + 1u];
        output_f16[out_base + 2u] = embeddings6[embed_base + 2u];
        output_f16[out_base + 3u] = embeddings6[embed_base + 3u];
    } else if (token_id < section_end) {
        local_token = token_id - section7_start;
        embed_base = local_token * u.input_hidden_size + dim_base;
        output_f16[out_base] = embeddings7[embed_base];
        output_f16[out_base + 1u] = embeddings7[embed_base + 1u];
        output_f16[out_base + 2u] = embeddings7[embed_base + 2u];
        output_f16[out_base + 3u] = embeddings7[embed_base + 3u];
    } else {
        write_zero(out_base);
    }
}
