...
@group(0) @binding(4) var<storage, read_write> output_f32: array<f32>;
...
fn read_weight(row_base: u32, dim: u32) -> f32 {
    let byte_value = packed_byte(row_base + (dim >> 1u));
    return signed_int4(byte_value, (dim & 1u) == 1u) * WEIGHT_SCALE;
}

@compute @workgroup_size(WORKGROUP_SIZE_VEC4, 1, 1)
fn gather_litert_int4_vec4_f32_out(@builtin(global_invocation_id) gid: vec3<u32>) {
    let tid = gid.x;
    ...
    output_f32[out_base] = read_weight(row_base, dim_base);
    output_f32[out_base + 1u] = read_weight(row_base, dim_base + 1u);
    output_f32[out_base + 2u] = read_weight(row_base, dim_base + 2u);
    output_f32[out_base + 3u] = read_weight(row_base, dim_base + 3u);
}
