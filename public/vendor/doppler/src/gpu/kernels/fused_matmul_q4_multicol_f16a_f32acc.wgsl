// Fused Q4_K Matmul Kernel - W4A16 (F16 IO, F32 accumulator, multicol)
//
// Computes C_f16 = A * dequant(B_q4k) for M=1 GEMV with f16 activations.
// Identical to fused_matmul_q4_multicol_f16a.wgsl except the per-thread
// accumulator and shared-memory reduction run in f32 instead of f16. On
// AMD RDNA3 the f16-accumulator path of the f16a kernel measured ~60%
// slower than this f32-accumulator twin at FFN gemv shape. Q4K weight
// reads stay 4-bit (bandwidth win) and IO stays f16 (memory layout
// unchanged); only the reduction precision changes.
        ...
        for (var b: u32 = tid_in_col; b < full_blocks; b = b + THREADS_PER_COL_GEMV) {
            ...
            for (var sb: u32 = 0u; sb < 8u; sb = sb + 1u) {
                ...
                for (var i: u32 = 0u; i < SUBBLOCK_SIZE; i = i + 4u) {
                    ...
                    partial_sum = partial_sum + f32(a0) * f32(w0) + f32(a1) * f32(w1) + f32(a2) * f32(w2) + f32(a3) * f32(w3);
                }
            }
        }
