(function attachSimulatteUniverseGrounder(root, factory) {
  ...
  function groundUniverseGraph(input = {}) {
    ...
    for (const span of spanRows) {
      ...
      const node = {
        ...
        supportOnly: row.best.supportOnly === true,
        directlyGrounded: row.best.supportOnly !== true && row.best.identityEvidence === true,
      };
      nodes.push(node);
  ...
  function addPromptOwnedCandidateNodes(nodes, seen, candidateRows, input) {
    ...
    const rows = (candidateRows || [])
      .filter((row) => row && row.identityEvidence === true && row.supportOnly !== true)
      .filter((row) => String(row.indexName || '') === 'prompt-typed-slot')
  ...
  function addPromptIdentityCandidateNodes(nodes, seen, candidateRows, input, rejected) {
    ...
    const rows = (candidateRows || [])
      .filter((row) => row && row.supportOnly !== true && String(row.indexName || '') === 'semantic-surface-grounder')
      .sort((a, b) => Number(b.confidence || 0) - Number(a.confidence || 0));
    let added = 0;
