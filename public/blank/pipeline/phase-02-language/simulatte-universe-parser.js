(function attachSimulatteUniverseParser(root, factory) {
  ...
  function mergeCompatibleOverlappingSpans(spans = [], lower = '') {
    const output = [];
    for (const span of spans.slice().sort((a, b) => a.start - b.start || b.end - a.end)) {
      const existingIndex = output.findIndex((row) => spansDescribeOneConcept(row, span));
      if (existingIndex < 0) {
        output.push(span);
        continue;
      }
      const existing = output[existingIndex];
      const start = Math.min(existing.start, span.start);
      const end = Math.max(existing.end, span.end);
      const primary = existing.end - existing.start >= span.end - span.start ? existing : span;
      output[existingIndex] = {
        ...primary,
        text: lower.slice(start, end),
        start,
        end,
        tokenStart: Math.min(existing.tokenStart, span.tokenStart),
        tokenEnd: Math.max(existing.tokenEnd, span.tokenEnd),
        entityClass: existing.entityClass === span.entityClass
  ...
  function spansDescribeOneConcept(left = {}, right = {}) {
    if (!['entity', 'environment'].includes(left.kind) || left.kind !== right.kind) return false;
    if (left.preserveNestedSemanticSpans === true || right.preserveNestedSemanticSpans === true) return false;
    if (!(left.start < right.end && right.start < left.end)) return false;
    const sameArchetype = left.visualArchetype && left.visualArchetype === right.visualArchetype;
    const sameClass = left.entityClass && left.entityClass === right.entityClass;
    const sameGrammar = left.localGeometryGrammarId && left.localGeometryGrammarId === right.localGeometryGrammarId;
    return Boolean(sameArchetype || sameClass || sameGrammar);
  }
  ...
  function buildClauses(spans, lower) {
    ...
    const entities = clauseEntities.filter((span) => (
      !attributiveMaterials.has(span.id) && !spanIsNegated(lower, span, processes)
    ));
    const clauses = [];
  ...
  function spatialClausesForEntities(entities = [], processes = [], lower = '') {
    ...
    for (let index = 1; index < ordered.length; index += 1) {
      ...
      if (!processBetween && isCompoundContainmentBridge(bridge) &&
        previous.semanticRole === 'containing-environment' && target.semanticRole !== 'containing-environment') {
        clauses.push({
          ...spatialClause(target, previous, null, ['inside']),
          relationSource: 'data-owned-compound-containment',
        });
        continue;
      }
      if (prepositions.length) {
        const targetSuffixEnd = ordered[index + 1] ? ordered[index + 1].start : lower.length;
        const targetSuffix = lower.slice(target.end, Math.min(targetSuffixEnd, target.end + 24));
        if (prepositions.includes('with') && (
          target.semanticRole === 'part' || target.semanticRole === 'lighting-environment' ||
          /\boutside\b/.test(targetSuffix)
        )) continue;
    ...
    clauses.push(...dataOwnedContainmentClauses(ordered));
    return clauses;
  }

  function dataOwnedContainmentClauses(entities = []) {
    const clauses = [];
    for (const child of entities.filter((span) => span.containerClass)) {
      const owner = entities.filter((span) => (
        span !== child && span.entityClass === child.containerClass
      )).sort((left, right) => (
        Math.abs(left.start - child.start) - Math.abs(right.start - child.start)
      ))[0];
      if (!owner) continue;
      clauses.push({
        ...spatialClause(child, owner, null, ['inside']),
        relationSource: 'data-owned-container-class',
      });
    }
    for (const child of entities.filter((span) => span.containedByFamily)) {
      const owner = entities.filter((span) => (
        span !== child && span.containerFamily === child.containedByFamily
      )).sort((left, right) => (
        Math.abs(left.start - child.start) - Math.abs(right.start - child.start)
      ))[0];
      if (!owner) continue;
      clauses.push({
        ...spatialClause(child, owner, null, ['inside']),
        relationSource: 'data-owned-container-family',
      });
    }
    return clauses;
  }

  function isCompoundContainmentBridge(bridge = '') {
    return /^\s*(?:(?:with|containing)\s+)?$/.test(bridge);
  }
  ...
  function spanIsNegated(lower, span, processes = []) {
    if (!span || !Number.isFinite(span.start)) return false;
    const source = String(lower || '');
    const start = Math.max(0, span.start - 64);
    const prefix = source.slice(start, span.start);
    if (new RegExp(`${NEGATION_RE.source}\\s+$`).test(prefix)) return true;
    const matcher = new RegExp(`\\b(?:${NEGATION_WORDS.join('|')})\\b`, 'g');
    let match = matcher.exec(prefix);
    let last = null;
    while (match) {
      last = match;
      match = matcher.exec(prefix);
    }
