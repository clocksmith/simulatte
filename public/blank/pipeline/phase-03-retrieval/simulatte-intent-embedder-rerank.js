  ...
  with (scope) {
    ...
    async function verifyPromptRuntimeReranker(runtime, provider, options = {}) {
        ...
        const base = {
          ...
          selectedCandidateId: config.qualification && config.qualification.selectedCandidateId || null,
          modelExecuted: false,
          modelNotExecutedReason: config.qualification && config.qualification.modelNotExecutedReason || '',
          probeCount: 0,
    ...
    function promptRuntimeReceipt(runtime, provider, details = {}) {
        ...
        return {
          ...
          rerankerQualificationStatus: reranker.qualification && reranker.qualification.status || '',
          rerankerSelectedCandidateId: reranker.qualification && reranker.qualification.selectedCandidateId || null,
          rerankerModelExecuted: rerankerProbe.ready === true,
          rerankerModelNotExecutedReason: rerankerProbe.ready === true
            ? null
            : reranker.qualification && reranker.qualification.modelNotExecutedReason || 'reranker-not-executed',
    ...
    function modelSummary(runtime, query, provider) {
        ...
        return {
          ...
          universeDocuments: runtime.universe ? runtime.universe.documentCount : 0,
          reranker: reranker.enabled ? reranker.id : null,
          rerankerCandidateId: reranker.id,
          rerankerKind: reranker.kind,
          rerankerPhase: 3,
          rerankerEnabled: reranker.enabled === true,
          ...
          rerankerQualificationStatus: reranker.qualification && reranker.qualification.status || '',
          rerankerModelNotExecutedReason: reranker.enabled
            ? null
            : reranker.qualification && reranker.qualification.modelNotExecutedReason || 'reranker-disabled',
          rerankerFallbackMode: reranker.fallbackMode,
          backend: provider && provider.backend || '',
        };
      }
    ...
    function blankResult(runtime) {
        ...
        return {
          ...
          rerank: {
            ...
            modelNotExecutedReason: reranker.enabled
              ? 'blank-prompt'
              : reranker.qualification && reranker.qualification.modelNotExecutedReason || 'reranker-disabled',
