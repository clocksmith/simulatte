  ...
  const scope = root.SimulattePhaseModuleRegistry.family('physicsModel');
    ...
    function retrievalGroundingEvidence(
        ...
        return {
          ...
          assumptions: [],
          unsupported: phaseCarryObject((missingRequiredSlots || [])
            .filter((row) => row.status === 'unsupported')
            .map((row) => ({
              id: `unsupported:${row.entryId || row.slotId}`,
              label: row.entryId || row.slotId || 'required action',
              reason: row.reason || 'required retrieval capability unavailable',
    ...
    function activationCloudFromPhase3Artifact(artifact = {}) {
        ...
        const intentBriefWithEvidence = {
          ...
          const obligationVerdicts = allObligationVerdicts;
