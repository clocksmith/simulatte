(function attachSimulattePhysicsModelphaseruntimelanguage(root) {
  const scope = root.SimulattePhaseModuleRegistry.family('physicsModel');
    ...
    function runPhase8SceneProof(phase7Output, options = {}) {
        ...
        return scope.assertPhaseEnvelope(api.settleSceneProof(phase7Output, options), 8, 'Phase 8 output');
      }
    ...
    function withPhase1RetrievalEvidence(phase1Output, retrievalEvidence = {}) {
        scope.assertPhaseEnvelope(phase1Output, 1, 'Phase 1 retrieval carrier');
    ...
    function stableTextHash(value = '') {
          return `fnv1a:${scope.fnv1a32(value || '').toString(16).padStart(8, '0')}`;
        }
    ...
    function sceneLanguageGraphFromLanguageGraph(languageGraph = {}) {
      ...
      const entities = scope.uniqueById([
        ...spans
      ...
      const parts = scope.uniqueById(spans
        .filter((span) => span.kind === 'entity' && span.semanticRole === 'part')
        .map((span) => sceneEntryForSpan(span, 'part', languageGraph)));
          const actions = scope.uniqueById([
          ...
          const environments = scope.uniqueById(spans
            .filter((span) => span.kind === 'environment')
            .map((span) => sceneEntryForSpan(span, 'environment', languageGraph)));
          const promptMediums = spans
            .filter((span) => span.kind === 'material' && scope.WATER_MEDIUM_RE.test(scope.normalizeForEvidence(span.text || span.materialHint || '')))
          ...
          const inferredMediums = predicates.some((predicate) => (
          ...
          )) || environments.some((entry) => scope.WATER_ENVIRONMENT_RE.test(scope.normalizeForEvidence(entry.label)))
            ? [{
              id: 'medium:water',
              kind: 'medium',
              label: 'water',
            ...
            : [];
          const mediums = scope.uniqueById([...promptMediums, ...inferredMediums]);
        ...
        span.kind === 'term' && !representedTermSpanIds.has(span.id)
          ...
          return {
            schema: scope.SCENE_LANGUAGE_GRAPH_SCHEMA,
      ...
      parts,
            ...
            relations,
            negations,
            unsupportedSpans: unsupportedSpans.map((span) => scope.phaseCarryObject({
              id: span.id || '',
              text: span.text || '',
    ...
    function sceneTargetForSpan(span = {}, kind = '') {
          if (span.entityClass) return scope.normalizeForEvidence(span.entityClass).replace(/\s+/g, '-');
          if (kind === 'action') {
            const text = scope.normalizeForEvidence(span.text);
            if (scope.SWIMMING_RE.test(text)) return 'swimming';
            return text.replace(/\s+/g, '-') || 'action';
          }
          if (kind === 'medium') {
            if (span.materialHint) return scope.normalizeForEvidence(span.materialHint).replace(/\s+/g, '-');
            if (scope.WATER_MEDIUM_RE.test(scope.normalizeForEvidence(span.text))) return 'water';
          }
          return scope.normalizeForEvidence(span.text).replace(/\s+/g, '-') || kind || 'entry';
        }
    ...
    function sceneSpanHasKnownRole(span = {}) {
      ...
      function sceneRelationsFromLanguageGraph(languageGraph = {}) {
        ...
        const actionTarget = predicate.process ? scope.normalizeForEvidence(predicate.process).replace(/\s+/g, '-') : '';
            const implicitTarget = predicate.implicitObject
              ? scope.normalizeForEvidence(predicate.implicitObject).replace(/\s+/g, '-')
              : '';
  ...
  function sceneNodeIdForSpan(languageGraph = {}, span = {}) {
    ...
    function sceneNegationsFromLanguageGraph(languageGraph = {}) {
          return (languageGraph.negations || []).map((negation, index) => scope.phaseCarryObject({
    ...
    function sceneSpanIsNegated(languageGraph = {}, span = null) {
      ...
      return negations.some((negation) => {
        const negToken = Number.isInteger(negation.tokenStart) ? negation.tokenStart :
          (languageGraph.tokens || []).findIndex((token) => token.start === negation.start);
        if (spanTokenStart >= 0 && negToken >= 0) {
          const distance = spanTokenStart - negToken;
    ...
    function queryPlanFromSceneLanguageGraph(sceneLanguageGraph = {}) {
          const slots = [];
          const addSlot = (slot) => slots.push(scope.phaseCarryObject(slot));
      ...
      for (const relation of sceneLanguageGraph.relations || []) {
        ...
        addSlot({
          ...
          const visualTargets = scope.uniqueList((sceneLanguageGraph.actions || [])
            .filter((entry) => entry.negated !== true)
            .flatMap((entry) => typeof scope.visualSlotTargetsForAction === 'function' ? scope.visualSlotTargetsForAction(entry) : []));
          ...
          return {
            schema: scope.SCENE_QUERY_PLAN_SCHEMA,
            sourcePromptHash: sceneLanguageGraph.sourcePromptHash || '',
            slots: scope.uniqueById(slots),
            summary: {
              slotCount: scope.uniqueById(slots).length,
              requiredSlotCount: scope.uniqueById(slots).filter((slot) => slot.required !== false).length,
            },
          };
      }
    ...
    function phase2CompositionLedger(sceneLanguageGraph = {}, queryPlan = {}, phase1Ledger = null) {
          const entries = scope.uniqueById([
      ...
      ...(sceneLanguageGraph.parts || []),
        ...
        ownedByPhase: slot.slotRole === 'visual' || slot.slotRole === 'part' ? 6 :
            ...
            mustPreserveIds: scope.uniqueList([
              slot.entryId,
              ...(slot.relationIds || []),
            ].filter(Boolean)),
    ...
    function normalizeCompositionLedger(ledger = {}, overrides = {}) {
          const next = {
            ...
            ...overrides,
            schema: scope.SCENE_COMPOSITION_LEDGER_SCHEMA,
            entries: scope.uniqueById([...(overrides.entries || []), ...(ledger.entries || [])]),
            relations: scope.uniqueById([...(overrides.relations || []), ...(ledger.relations || [])]),
            obligations: scope.uniqueById([...(overrides.obligations || []), ...(ledger.obligations || [])]),
          ...
          next.summary = {
            ...
            failedCount: obligations.filter((row) => scope.LEDGER_FAILURE_STATUSES.has(row.status)).length,
          };
          return scope.phaseCarryObject(next);
        }
    ...
    function runPhase1RuntimeGate(sourceText = '', options = {}) {
          ...
          const receipts = scope.phase1RuntimeReceipts(runtimeContext, options);
          const compositionLedger = phase1CompositionLedger(promptText);
          return scope.createPhaseEnvelope({
            phase: 1,
            inputSchema: scope.PHASE_ZERO_INPUT_SCHEMA,
    ...
    root.SimulattePhaseModuleRegistry.define('physicsModel', 'simulatte-physics-model-phase-runtime-language.js', {
      sceneProofApi,
      runPhase8SceneProof,
      phaseArtifactSet,
      mergePhaseArtifacts,
