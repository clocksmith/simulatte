  ...
  with (scope) {
    ...
    function addCouplingOperator(operators, type, from, to, params, edge) {
        ...
        if (type === 'particle_deposition') {
          return addOperator(operators, type, to, {
            reads: [`airborneDensity:${toEntity}`, `depositedMass:${toEntity}`],
            writes: [`airborneDensity:${toEntity}`, `depositedMass:${toEntity}`],
            params: { rate: clamp(Number(params.settlingRate || params.flowRate || 0.32), 0.02, 2) },
          });
        }
        return addOperator(operators, type, to, {
          reads: [],
