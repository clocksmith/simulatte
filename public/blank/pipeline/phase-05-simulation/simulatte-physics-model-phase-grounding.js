(function attachSimulattePhysicsModelphasegrounding(root) {
  const scope = root.SimulattePhaseModuleRegistry.family('physicsModel');
            ...
            receipts: [
              ...
              renderInstances: visualCompile.renderInstances.length,
                ...
                lostObligations: visualCompile.compositionLedger && Array.isArray(visualCompile.compositionLedger.obligations)
                  ? visualCompile.compositionLedger.obligations.filter((row) => scope.LEDGER_FAILURE_STATUSES.has(row.status)).length
                  : 0,
    ...
    function createRenderExecutionInput(source = {}, simulationState = null, canvas = null) {
        ...
        if (!phase6Output) {
          throw new Error(`renderExecutionInput source expected ${scope.phaseOutputSchema(6)}, received ${source && source.schema || 'missing phase6 artifact'}`);
        }
    ...
    function runPhase7RenderExecution(source, simulationState = null, canvas = null, frameReceipt = {}) {
        ...
        if (source && source.schema === scope.RENDER_EXECUTION_INPUT_SCHEMA) {
          if (source.inputSchema !== scope.phaseOutputSchema(6)) {
            throw new Error(`Phase 7 input expected ${scope.phaseOutputSchema(6)}, received ${source.inputSchema || 'missing inputSchema'}`);
          }
