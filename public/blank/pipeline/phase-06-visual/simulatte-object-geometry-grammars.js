(function attachSimulatteObjectGeometryGrammars(root) {
  const scope = root.SimulattePhaseModuleRegistry.family('compositionGraph');
    ...
    const OBJECT_GEOMETRY_GRAMMARS = Object.freeze({
      cube: grammar('cube', [0.2, 0.2], 25, [
        part('body', 'hexagon', [0, 0], [0.86, 0.86], '#8dabbf', Math.PI / 6, 1, '', 'core'),
        part('edge-down', 'capsule', [0, 0.21], [0.42, 0.018], '#294b61', Math.PI / 2, 1, '', 'detail'),
        part('edge-left', 'capsule', [-0.182, -0.105], [0.42, 0.018], '#294b61', Math.PI / 6, 1, '', 'detail'),
        part('edge-right', 'capsule', [0.182, -0.105], [0.42, 0.018], '#294b61', -Math.PI / 6, 1, '', 'detail'),
      ]),
      ...
      warehouse: grammar('warehouse', [0.46, 0.38], 3, [
        part('industrial-shell', 'rounded-box', [0, 0.12], [0.96, 0.7], '#7b858c'),
        part('roof-left', 'triangle', [-0.27, -0.31], [0.52, 0.28], '#48535a'),
        part('roof-right', 'triangle', [0.27, -0.31], [0.52, 0.28], '#566168'),
        part('loading-bay-left', 'rounded-box', [-0.25, 0.21], [0.3, 0.38], '#27343d'),
        part('loading-bay-right', 'rounded-box', [0.17, 0.21], [0.3, 0.38], '#303e47'),
        part('bay-stripe-left', 'capsule', [-0.25, 0.04], [0.26, 0.035], '#d9b34c'),
        part('bay-stripe-right', 'capsule', [0.17, 0.04], [0.26, 0.035], '#d9b34c'),
        part('clerestory-left', 'rounded-box', [-0.25, -0.12], [0.22, 0.1], '#8bc7d2'),
        part('clerestory-right', 'rounded-box', [0.19, -0.12], [0.22, 0.1], '#8bc7d2'),
      ]),
      laboratory: grammar('laboratory', [0.42, 0.34], 3, [
        part('back-wall', 'rounded-box', [0, -0.08], [0.96, 0.76], '#d5e0e5', 0, 0.72),
        part('floor', 'rounded-box', [0, 0.42], [0.96, 0.16], '#71818c'),
        part('upper-shelf', 'rounded-box', [0.2, -0.35], [0.5, 0.08], '#51616c'),
        part('bench-top', 'rounded-box', [-0.08, 0.1], [0.78, 0.13], '#8b9ca5'),
        part('bench-leg-left', 'rounded-box', [-0.37, 0.3], [0.09, 0.43], '#55636c'),
        part('bench-leg-right', 'rounded-box', [0.22, 0.3], [0.09, 0.43], '#55636c'),
        part('monitor-case', 'rounded-box', [0.22, -0.1], [0.28, 0.27], '#263642'),
        part('monitor-screen', 'rounded-box', [0.22, -0.11], [0.22, 0.17], '#62c3d6'),
        part('flask-body', 'ellipse', [-0.27, -0.04], [0.2, 0.22], '#68c7bd', 0, 0.78),
        part('flask-neck', 'rounded-box', [-0.27, -0.19], [0.07, 0.18], '#8adbd2', 0, 0.82),
        part('microscope-base', 'rounded-box', [0.02, 0], [0.22, 0.08], '#344550'),
        part('microscope-arm', 'capsule', [0.03, -0.12], [0.28, 0.08], '#7c8d96', -0.85),
        part('microscope-lens', 'capsule', [0.12, -0.23], [0.16, 0.07], '#293943', 1.2),
        part('sample-tray', 'rounded-box', [0.02, -0.01], [0.23, 0.06], '#d0b668'),
      ]),
      ...
      'smoke-cloud': grammar('smoke-cloud', [0.25, 0.22], 31, [
        part('plume-base', 'ellipse', [0, 0.28], [0.52, 0.3], '#4d5257', 0, 0.9),
        part('plume-left', 'ellipse', [-0.2, 0.02], [0.52, 0.48], '#626970', -0.12, 0.82),
        part('plume-right', 'ellipse', [0.2, -0.08], [0.5, 0.5], '#747c84', 0.14, 0.76),
        part('plume-top', 'ellipse', [-0.02, -0.34], [0.46, 0.42], '#8a9299', 0, 0.66),
        part('soot-core', 'wave', [0, -0.03], [0.46, 0.8], '#34383c', 1.5, 0.54),
      ]),
      'fire-front': grammar('fire-front', [0.22, 0.28], 34, [
        part('ember-bed', 'ellipse', [0, 0.34], [0.72, 0.2], '#d44725', 0, 0.82),
        part('flame-left', 'triangle', [-0.22, 0.02], [0.38, 0.72], '#e75b2a', -0.12, 0.9),
        part('flame-center', 'triangle', [0, -0.12], [0.42, 0.92], '#f39b32', 0.04, 0.94),
        part('flame-right', 'triangle', [0.23, 0.05], [0.34, 0.68], '#e8652b', 0.14, 0.88),
        part('hot-core', 'ellipse', [0, 0.16], [0.28, 0.46], '#ffd76b', 0, 0.84),
        part('spark-left', 'ellipse', [-0.27, -0.35], [0.06, 0.08], '#ffd878'),
        part('spark-right', 'ellipse', [0.28, -0.26], [0.05, 0.07], '#ffb83f'),
      ]),
      'soot-deposit': grammar('soot-deposit', [0.24, 0.14], 35, [
        part('deposit-band', 'wave', [0, 0.04], [0.94, 0.38], '#25282a', -0.04, 0.82),
        part('deposit-smear', 'capsule', [-0.08, 0.11], [0.72, 0.09], '#34383a', -0.08, 0.72),
        part('grain-left', 'ellipse', [-0.32, -0.08], [0.1, 0.1], '#181a1b', 0, 0.88),
        part('grain-mid', 'ellipse', [-0.04, -0.13], [0.08, 0.08], '#202224', 0, 0.84),
        part('grain-right', 'ellipse', [0.29, -0.04], [0.11, 0.11], '#141617', 0, 0.9),
      ]),
