(function attachSimulatteCompositionGraphConstraintLayout(root) {
  ...
  with (scope) {
    ...
    function layoutConstructionContainer(object = {}) {
      const construction = object.construction || {};
      const evidence = [
        object.kind,
        ...
        ...(construction.relationHints || []),
      ].join(' ').toLowerCase().replace(/_/g, '-');
      return /\b(?:container|enclosure|facility|folded-surface|habitat|interior|matrix|medium|porous-matrix|room|volume)\b/.test(evidence);
    }
    ...
    function layoutRelationsForSpec(spec = {}, states = []) {
      ...
      for (const relation of ledger.relations || []) {
        ...
        const deposition = relation.process === 'deposition' ||
          /\b(?:coat|coats|coating|deposit|deposits|depositing|layer|layers|layering)\b/.test(predicate);
        ...
        } else if (!spatialRelation && /\b(?:exchange|exchanges|exchanging|transfer|transfers|transferring|diffuse|diffuses|diffusing)\b/.test(predicate)) {
          spatialRelation = 'near';
      ...
      const specificallyConstrainedPairs = new Set(resolvedRows
        .filter((row) => row.spatialRelation !== 'with')
        .map((row) => [row.from.object.id, row.to.object.id].sort().join(':')));
      const byConstraint = new Map();
      for (const row of resolvedRows) {
        const pair = [row.from.object.id, row.to.object.id].sort().join(':');
        if (row.spatialRelation === 'with' && specificallyConstrainedPairs.has(pair)) continue;
    ...
    function layoutObjectForReference(states = [], reference = '') {
      ...
      if (instanceRef) {
        ...
        if (!instanceRef[3]) {
          const exactConstructionState = states.find((state) => {
            const construction = state.object.construction || {};
            return [construction.targetEntryId, ...(construction.sourceCardIds || [])]
              .some((value) => String(value || '').toLowerCase() === String(reference || '').toLowerCase());
          });
          if (exactConstructionState) return exactConstructionState;
        }
      }
      ...
      const ranked = states.map((state) => {
        const tokens = layoutIdentityTokens([
          ...
          state.object.semanticClass, state.object.visualArchetype,
    ...
    function applySpatialConstraint(relation, states, applied) {
      ...
      if (['in', 'inside', 'into', 'within'].includes(type)) {
        ...
        a.y += (b.y + (a.h + b.h) * 0.72 - a.y) * 0.64;
      } else if (type === 'with' && (layoutConstructionContainer(a.object) || layoutConstructionContainer(b.object))) {
        const outer = layoutConstructionContainer(a.object) ? a : b;
        const inner = outer === a ? b : a;
        outer.container = true;
        outer.w = Math.max(outer.w, Math.min(0.72, inner.w + 0.2));
        outer.h = Math.max(outer.h, Math.min(0.62, inner.h + 0.18));
        inner.w = Math.min(inner.w, outer.w * 0.72);
        inner.h = Math.min(inner.h, outer.h * 0.72);
        inner.x += (outer.x + direction * outer.w * 0.08 - inner.x) * 0.72;
        ...
        a.h = Math.min(Math.max(a.h, b.h * 0.18), b.h * 0.46);
        a.x += (b.x - a.x) * 0.82;
        a.y += (b.y + b.h * 0.08 - a.y) * 0.82;
        a.z = (Number.isFinite(b.z) ? b.z : 0) - 0.75;
        b.z = Number.isFinite(b.z) ? b.z : 0;
    ...
    function enforceHardContainment(relations = []) {
      for (const relation of relations) {
        ...
        if (['in', 'inside', 'into', 'within'].includes(relation.spatialRelation)) {
          inner = relation.from;
          outer = relation.to;
        } else if (relation.spatialRelation === 'with') {
          const fromContainer = layoutConstructionContainer(relation.from.object);
          const toContainer = layoutConstructionContainer(relation.to.object);
          if (fromContainer === toContainer) continue;
          outer = fromContainer ? relation.from : relation.to;
          inner = outer === relation.from ? relation.to : relation.from;
        }
        if (!inner || !outer) continue;
        const margin = 0.012;
        outer.container = true;
        outer.w = Math.max(outer.w, Math.min(0.82, inner.w + margin * 2));
        outer.h = Math.max(outer.h, Math.min(0.72, inner.h + margin * 2));
        inner.w = Math.min(inner.w, Math.max(0.04, outer.w - margin * 2));
        inner.h = Math.min(inner.h, Math.max(0.04, outer.h - margin * 2));
        inner.x = clampLayoutNumber(
          inner.x,
          outer.x - outer.w * 0.5 + inner.w * 0.5 + margin,
          outer.x + outer.w * 0.5 - inner.w * 0.5 - margin
        );
        inner.y = clampLayoutNumber(
          inner.y,
          outer.y - outer.h * 0.5 + inner.h * 0.5 + margin,
          outer.y + outer.h * 0.5 - inner.h * 0.5 - margin
        );
        inner.z = (Number.isFinite(outer.z) ? outer.z : 0.5) - 0.5;
        outer.z = Number.isFinite(outer.z) ? outer.z : 0.5;
      }
    }
