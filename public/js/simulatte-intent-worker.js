(function attachSimulatteIntentWorker(root) {
  ...
  function createEmbedder(config = {}) {
    ...
    try {
      ...
      embedder = root.SimulatteIntentEmbedder.create({
        ...
        manifestUrl: config.manifestUrl || '../models/simulatte-embedder/manifest.json',
  ...
  async function handleLoad(data = {}) {
    ...
    try {
      await createEmbedder(data.config || {}).loadModel();
      post('simulatte:intent-worker:result', data.id, { ok: true, result: { ready: true } });
    } catch (error) {
