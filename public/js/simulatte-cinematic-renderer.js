(function attachSimulatteCinematicRenderer(root, factory) {
  ...
  class CinematicRenderer {
    ...
    setSpec(spec) {
      const brief = spec && spec.intent && spec.intent.intentBrief || null;
      const visual = brief && brief.visualIntent || {};
      this.sceneKind = visual.sceneKind || sceneKindFromSpec(spec);
      this.sceneId = SCENE_IDS[this.sceneKind] ?? SCENE_IDS[sceneKindFromSpec(spec)] ?? 3;
      this.affordanceText = [
        visual.geometry,
        ...(visual.shaderHints || []),
        ...(visual.motionHints || []),
        ...((visual.affordances || []).map((row) => [
          row.id,
      ...
      this.metrics = metricsForSpec(spec, visual, this.affordanceText);
      this.canvas.style.opacity = spec && spec.templateId !== 'blank-world' ? '0.92' : '0';
    }

    render(state, spec, nowMs) {
      if (!spec || spec.templateId === 'blank-world') {
      ...
      gl.uniform1f(this.uniforms.uSeed, seedForSpec(spec));
  ...
  function sceneKindFromSpec(spec) {
    const text = [
      ...
      spec && spec.intent && spec.intent.prompt,
      spec && spec.name,
      (spec && spec.modules || []).join(' '),
  ...
  function metricsForSpec(spec, visual, text) {
    const params = spec && spec.params || {};
    const read = (key, fallback) => Number.isFinite(Number(params[key])) ? Number(params[key]) : fallback;
    return {
      heat: clamp01(read('heatTransfer', /thermal|heat|lava|fire|battery|chip/.test(text) ? 0.82 : 0.38)),
      flow: clamp01(read('flowRate', /flow|wind|water|plume|droplet|pressure/.test(text) ? 0.78 : 0.46)),
      density: clamp01(read('density', /crowd|bio|smoke|plume|particles|network/.test(text) ? 0.76 : 0.48)),
  ...
  function seedForSpec(spec) {
    const text = `${spec && spec.id || ''}:${spec && spec.name || ''}:${spec && spec.intent && spec.intent.prompt || ''}`;
    let hash = 2166136261;
