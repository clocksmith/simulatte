(function activationCloudModule(root, factory) {
  ...
  const LANGUAGE_VISUAL_SIGNAL_RULES = Object.freeze([
    signalRule('thermal', 'thermal operator signal', /\b(heat|thermal|temperature|cool|cools|cooled|cooling|coolant|steam|lava|hot|cold|melt|freeze|fire|flame|smoke)\b/, ['heat', 'thermal', 'phase'], ['thermal-gradient', 'phase-boundary', 'emission'], ['temperature', 'heat_transfer']),
    signalRule('fluid', 'fluid operator signal', /\b(flow|fluid|water|river|wind|airflow|coolant|pump|channel|droplet|gas|bubble|bubbles|pressure|velocity|turbulence|vortex)\b/, ['flow', 'pressure', 'advection'], ['streamline', 'flow-ribbon', 'tracer'], ['flowVelocity', 'pressure']),
    signalRule('stress', 'stress operator signal', /\b(stress|strain|fracture|crack|impact|collision|load|buckling|contact|deform|shear|torque|friction|centripetal)\b/, ['stress', 'contact', 'constraint'], ['crack-network', 'strain-field', 'contact-pad'], ['stress', 'damage']),
    ...
    signalRule('acoustic', 'acoustic operator signal', /\b(acoustic|sound|standing wave|standing waves|frequency|speaker|vibration|pressure ring|air pressure wave|air pressure waves)\b/, ['wave', 'pressure', 'resonance'], ['pressure-ring', 'standing-node', 'waveguide'], ['amplitude', 'pressure']),
    signalRule('biological', 'biological operator signal', /\b(biological|biofilm|cell|protein|root|coral|algae|mycelium|membrane|neuron|tissue|microbiome|microbe|enzyme|ferment|fermentation|yeast|sourdough|gluten|dough|compost|greenhouse|nutrient|biomass|crop|plant|mangrove|kelp|plankton)\b/, ['growth', 'diffusion', 'density'], ['branching-network', 'membrane-sheet', 'cell-cluster'], ['density', 'nutrient']),
    signalRule('chemical', 'chemical operator signal', /\b(reaction|chemical|acid|acidic|acidity|crystal|concentration|electrolyte|solvent|catalyst|reagent|diffusion|dose|ferment|fermentation|gas)\b/, ['reaction', 'diffusion', 'concentration'], ['reaction-front', 'diffusion-cloud', 'crystal-facet'], ['reactionProgress', 'concentration']),
    ...
    signalRule('surface', 'cryosphere surface signal', /\b(glacier|iceberg|fjord|sea ice|ice shelf|ice cliff|calving|meltwater|cryosphere)\b/, ['phase', 'fluid', 'stress'], ['ice-cliff', 'calving-block', 'fjord-sheet'], ['phase', 'flowVelocity', 'stress']),
    signalRule('motion', 'sport motion signal', /\b(skate|skateboard|rider|bowl|carve|carves|centripetal|trajectory|wheel|friction loss)\b/, ['motion', 'stress', 'constraint'], ['trajectory-arc', 'wheel-contact', 'curved-bowl'], ['velocity', 'contactForce']),
