(function(root){
  function create(scene,start,length,rate){
    ...
    const from=Math.max(0,start-(maximum*2+250)/c),count=Math.ceil((end-from)*rate)+3;
    ...
    function receive(point,seed=0,onlySource=null,moving=false){
      ...
      for(const source of onlySource?[onlySource]:scene.sources){
        ...
        for(let block=0;block<length;block+=160){
          const last=Math.min(length-1,block+160),times=[start+block/rate,start+last/rate];
          ...
          for(const id of ids){
            ...
            for(let i=block;i<Math.min(length,block+160);i++){
    ...
    return {receive,geometryStepSeconds:160/rate,emissionStart:from};
  }
