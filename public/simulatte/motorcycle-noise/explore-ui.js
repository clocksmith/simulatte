(function(root){
  function create({view,getScene,getTime,getSelected,getComparison,selectSource}){
    ...
    function inspectMarker(id){
      selectedMarker=id;inspectedSource=null;placing=false;$('add-receiver').setAttribute('aria-pressed','false');
    ...
    function inspectSource(id){selectedMarker=null;inspectedSource=id;selectSource(id);$('inspection').hidden=false;$('source-actions').hidden=false;$('receiver-actions').hidden=true;renderMarkers();paint();}
    function paint(){
    ...
    function showObserver(data){
      ...
      ctx.strokeStyle='#8da595';ctx.lineWidth=.5;for(const level of [40,60,80]){const y=canvas.height-(level-30)/70*canvas.height;ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(canvas.width,y);ctx.stroke();}
      ctx.strokeStyle='#c6e5aa';ctx.lineWidth=2;ctx.beginPath();history.forEach((row,i)=>{const x=i/79*canvas.width,y=canvas.height-Math.max(0,Math.min(1,(row.level-30)/70))*canvas.height;i?ctx.lineTo(x,y):ctx.moveTo(x,y);});ctx.stroke();
    }
    function init(scene){
      ...
      observerWorker=new Worker('./observer-noise-worker.js?v=mobile-audio-v1');
      ...
      worker=new Worker('./live-noise-worker.js?v=mobile-audio-v1');worker.postMessage({type:'init',scene});
    ...
    function update(now){
      ...
      if(!observerPending&&now>=observerNextAt&&!document.hidden){
        const observer=view.getObserver(),sampleTime=getTime(),key=JSON.stringify([sampleTime,observer,scene.config,scene.panel,scene.treatments,scene.treatmentsEnabled]);
        if(key!==observerKey){
          observerKey=key;observerPending=true;observerNextAt=now+200;
          observerWorker.postMessage({type:'sample',id:++observerRequest,time:sampleTime,observer,config:scene.config,panel:scene.panel,treatments:scene.treatments,treatmentsEnabled:scene.treatmentsEnabled});
        }
      }
      if(pending||now<nextAt||document.hidden)return;
      const focus=view.getFocus(),observer=view.getObserver(),time=getTime(),key=JSON.stringify([time,focus,observer,markers,scene.config,scene.panel,scene.treatments,scene.treatmentsEnabled]);
      if(key===lastRequestKey)return;lastRequestKey=key;pending=true;nextAt=now+700;
      worker.postMessage({type:'sample',id:++requestId,time,focus,observer,markers,config:scene.config,panel:scene.panel,receiver:scene.receiver,speaker:scene.speaker,reference:scene.reference,treatments:scene.treatments,treatmentsEnabled:scene.treatmentsEnabled});
    }
    function pick(value){
      if(treatments.pick(value)){selectedMarker=null;inspectedSource=null;renderMarkers();return true;}
      ...
      if(value.point){view.placeObserver(value.point,value.surface||'sidewalk');$('camera-mode').value=value.surface||'sidewalk';nextAt=0;return true;}
      return false;
    }
    ...
    on($('park-home'),'click',()=>{view.homePark();$('camera-mode').value='map';$('area-focus').value='McCarren Park';nextAt=0;observerNextAt=0;});
    ...
    on($('inspection-close'),'click',()=>{selectedMarker=null;inspectedSource=null;$('inspection').hidden=true;renderMarkers();});
    ...
    return {update,pick,dispose(){treatments.dispose();epoch++;events.abort();worker?.terminate();observerWorker?.terminate();view.setReceiverMarkers([],null);}};
  }
