(function(root){
  const STEP=.1,STRIDE=9,STEPS=601;
  ...
  function pathFor(source,geometry){
    ...
    for(let i=0;i<segments.length;i++){
      ...
      const beginning=points[points.length-1],steps=Math.max(1,Math.ceil(length(beginning,end)/2));
      let obstructed=false;
      for(let j=1;j<=steps;j++){const p={x:beginning.x+(end.x-beginning.x)*j/steps,y:beginning.y+(end.y-beginning.y)*j/steps};if(geometry.occupied(p)){obstructed=true;break;}add(p,key);}
      ...
      if(curve.some(p=>geometry.occupied(p)))break;
  ...
  function prepare(sources,geometry){
    const agents=[];
    for(const source of sources){
      const path=pathFor(source,geometry);if(path.length<10)continue;
      let progress=Math.min(source.offset,path.length-5),point=at(path,progress),attempt=0;
      while(attempt++<24&&agents.some(other=>other.source.kind!=='pedestrian'&&source.kind!=='pedestrian'&&length(point,other.point)<7)){
        progress+=7;if(progress>Math.min(path.length-5,160))break;point=at(path,progress);
      }
      if(progress>path.length-5||geometry.occupied(point)||agents.some(other=>other.source.kind!=='pedestrian'&&source.kind!=='pedestrian'&&length(point,other.point)<7))continue;
      const states=new Float64Array(STEPS*STRIDE);source.motion={step:STEP,stride:STRIDE,states};
    ...
    for(let step=1;step<STEPS;step++){
      ...
      for(const agent of agents){
        ...
        if(!pedestrian)for(const other of agents){
          ...
          if(forward>0&&forward<50&&side<1.5&&Math.cos(other.point.heading-agent.point.heading)>.5){
        ...
        const actualSpeed=travel/STEP,acceleration=(actualSpeed-oldSpeed)/STEP,headingChange=Math.atan2(Math.sin(point.heading-agent.point.heading),Math.cos(point.heading-agent.point.heading));
  ...
  function sample(source,time){
    ...
    const t=Math.max(0,Math.min(60,time))/motion.step,low=Math.min(STEPS-2,Math.floor(t)),f=Math.min(1,t-low),a=low*STRIDE,b=(low+1)*STRIDE,rows=motion.states;
