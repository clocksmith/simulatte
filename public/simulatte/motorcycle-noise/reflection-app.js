(async function(root){
  ...
  let selected=null,placement=null,frameId=null,activeAudio=null,lastReadout=-Infinity;
  ...
  function stopAudio(){if(audioSource){audioSource.onended=null;audioSource.stop();audioSource.disconnect();audioSource=null;}$('listen').textContent='Listen';}
  ...
  function setPaused(value){paused=value;$('pause').textContent=paused?'Play':'Pause';}
  ...
  form.addEventListener('submit',event=>{event.preventDefault();try{const p=read();invalidate();scene=M.create(map,p);view.setSources(scene.sources);time=0;selected=scene.sources.find(row=>row.kind==='motorcycle')?.id;setPaused(false);status('New seeded traffic pass.');}catch(error){status(error.message,true);}});
  ...
  function run(){
    ...
    const id=generation;worker=new Worker('./reflection-worker.js');$('run').disabled=true;$('cancel').hidden=false;$('progress').hidden=false;$('progress').value=0;
  ...
  $('listen').addEventListener('click',async()=>{if(audioSource){stopAudio();return;}try{if(!activeAudio)return;if(!audio)audio=new AudioContext();await audio.resume();if(!activeAudio)return;
    const samples=activeAudio[$('audio-mode').value],buffer=audio.createBuffer(1,samples.length,result.sampleRate);
    buffer.getChannelData(0).set(Float32Array.from(samples,v=>Math.max(-.4,Math.min(.4,v*.02))));
    const source=audio.createBufferSource();source.buffer=buffer;source.connect(audio.destination);source.onended=()=>{if(audioSource===source){audioSource=null;$('listen').textContent='Listen';}source.disconnect();};audioSource=source;source.start();$('listen').textContent='Stop';
  }catch(error){status(error.message,true);}});$('audio-mode').addEventListener('change',stopAudio);
  $('export').addEventListener('click',()=>{if(!result)return;try{
    ...
    const spec=W.finalizeWorldSpec({id:'motorcycle-noise',kind:'nyc-noise-treatment-v2',templateId:'nyc-noise-treatment-v2',name:'NYC noise treatments',description:'Idealized redistribution, redirection and active cancellation.',params,objects:[],modules:[],controls:[],
      source:{schema:W.SOURCE_SCHEMA,prompt:'',compilerConfig:{adapter:'nyc-noise-treatment-v2'}},authorship:{schema:W.AUTHORING_SCHEMA,revision:0,sources:[{id:sourceId,authority:'userOverride',label:'Scenario controls'}],fieldProvenance:[{path:'/',authority:'userOverride',sourceId}],patches:[],reconciliations:[]},
    ...
    const url=URL.createObjectURL(new Blob([JSON.stringify({schema:'simulatte.nycNoiseReplay.v2',spec,record:result},null,2)],{type:'application/json'})),link=document.createElement('a');link.href=url;link.download=`nyc-noise-${scene.config.seed}.json`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);
  ...
  $('import').addEventListener('change',async event=>{try{
    ...
    const bundle=JSON.parse(await file.text());if(bundle.schema!=='simulatte.nycNoiseReplay.v2')throw new Error('Unsupported replay');root.SimulatteWorldSpec.validateWorldSpec(bundle.spec);
    const p=bundle.spec.params;if(bundle.spec.kind!=='nyc-noise-treatment-v2'||p.mapHash!==mapHash)throw new Error('Replay model or NYC map identity differs');M.validate(p.config);
    if(!Number.isFinite(p.time)||p.time<0||p.time>60)throw new Error('Replay time is outside this pass');
    ...
    invalidate();scene=M.create(map,p.config);for(const name of ['panel','receiver','reference','speaker'])scene[name]={...p[name]};scene.observers[0]={name:'Listener',...scene.receiver};time=p.time;showConfig(p.config);view.setSources(scene.sources);run();
  }catch(error){status(error.message,true);}event.target.value='';});
  function tick(now){
    if(!view)return;const dt=last?Math.min(.1,(now-last)/1000):0;last=now;
    if(!paused&&!document.hidden){time=Math.min(60,time+dt*Number($('playback').value));if(time>=60)setPaused(true);if(result)invalidate();}
  ...
  root.addEventListener('pagehide',()=>{cancelAnimationFrame(frameId);worker?.terminate();stopAudio();audio?.close();view?.dispose();view=null;});
  try{
    ...
    scene=M.create(map,{...M.defaults});showConfig(scene.config);selected=scene.sources.find(row=>row.kind==='motorcycle')?.id;
    status('Starting Babylon renderer');view=await root.MotorcycleReflectionView.create($('city'),map,scene,pick=>{if(placement&&pick.point)setEquipment(pick.point);else if(pick.sourceId){selected=pick.sourceId;$('selected').textContent=`Selected scenario source: ${selected}`;}});
    ...
    status('Ready. Orange travels outward; cyan returns after reaching the surface.');frameId=requestAnimationFrame(tick);
