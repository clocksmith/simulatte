  ...
  function create(scene,time){
    ...
    const sources=scene.sources.map(source=>{
      ...
      if(!spectra.has(key)){
        ...
        const measured=S.measure(waveform,rate).laeq;
        spectra.set(key,Number.isFinite(measured)?measured-M.sourceLevel(source,time):0);
      }
      return {source,position,weighting:spectra.get(key)};
    });
    function measure(point,includeAudio=false){
      ...
      for(const {source,position,weighting}of sources){
        ...
        for(const path of paths){
          ...
          const power=10**((M.sourceLevel(source,path.emissionTime)+weighting)/10);
