...
const out = path.join(root, 'artifacts/create-rearchitecture-baseline/worker-phase-browser');
...
try {
  ...
  const result = await evaluate(`(async () => {
    ...
    try {
      await instance.run(request, { resources: {
        ...
        renderer: { descriptor: descriptor('renderer', sourceDigest), handle: {
        ...
        frame: { index: 0, simulationTime: 0 }, viewport: { width: 1440, height: 1000 } } : {} });
  ...
  assert.notEqual(result.outputs[7].artifact.sceneProof.verdict, 'pass');
  retain('receipt.json', { schema: 'simulatte.workerPhaseBrowserAudit.v1',
    capturedAt: new Date().toISOString(), layer: 'actual-browser-worker-component',
    qualifiedForCreate: false, graphicsEvidence: 'diagnostic-only; no pixels submitted by this probe',
    sourceDigest, sources, result });
  console.log(JSON.stringify({ status: 'pass', output: path.relative(root, out), phases: 8,
    layer: 'actual-browser-worker-component', graphicsEvidence: 'not-proven' }));
} catch (error) {
