...
const root=fileURLToPath(new URL('../../',import.meta.url)).replace(/\/$/,''), out=root+'/'+(process.env.SIMULATTE_EVIDENCE_DIR||'artifacts/runtime-repair')+'/motorcycle';
...
const c=b.client, report={sources:await sourceReceipt(root),startedAt:new Date().toISOString(),cases:[],errors:[]};
...
try {
 ...
 for(const width of [1440,390]){
  ...
  await click('pause');
  const previousMeasurement=row.steps.at(-1).state.measurement?.identity.time??-1, samplingDeadline=Date.now()+30000;
  while(Date.now()<samplingDeadline){if(await ev(`globalThis.motorcycleMeasurementReceipt?.identity.time>${previousMeasurement}`))break;await wait(250);}
  row.steps.push({name:'onboard-running',state:await snap()});
  assert.ok(row.steps.at(-1).state.measurement?.identity.time>row.steps.at(-2).state.measurement?.identity.time,'tracked microphone continues sampling while moving');
  await click('pause');
