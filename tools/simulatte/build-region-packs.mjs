...
const DATA_DIR = path.join(ROOT, 'public/data/simulatte');
...
function updateManifest({ config, registry, registryPath, registrySha256, world, featureCatalog }) {
  ...
  fs.writeFileSync(MANIFEST_PATH, downloadText(manifest));
}

// Pretty canonical form. This is the world/feature *identity* serialization: the
// composition-hash anchors (manifest.world.sha256, registry.composition.*, and the
// check-autonomy-data validator) are all defined over this exact encoding, so it must
// stay pretty or every derived artifact that pins the world hash would have to change.
function artifactText(value) {
  return `${JSON.stringify(regionApi.sortValue(value), null, 2)}\n`;
}

// Compact canonical form for the files the browser actually downloads at boot (region
// packs, registry, manifest). Same sorted data, ~35% fewer bytes to transfer and parse.
// Pack/registry sha256s are taken over this encoding and verified against the same bytes
// at load, so compacting here is self-consistent and does not touch the identity anchors.
function downloadText(value) {
  return `${JSON.stringify(regionApi.sortValue(value))}\n`;
}
