...
const CACHE_DIR = path.join(ROOT, 'public/data/simulatte/cache');
...
const SOLAR_SYSTEM_FACTS = {
  schema: 'simulatte.solarSystemFacts.v1',
  ...
  facts: {
    ...
    bodies: {
      planets: 8,
      officiallyNamedDwarfPlanets: 5,
      knownMoons: 'hundreds',
      knownAsteroidsAndComets: 'thousands'
    },
    ageBillionYears: 4.6,
    outerBoundary: {
      source: 'Oort Cloud',
    ...
    MilkyWayOrbit: {
      periodYears: 230000000,
      speedMph: 515000
    },
    planetProperties: [
      { name: 'Mercury', diameterKm: 4880, meanDistanceKm: 58000000 },
      { name: 'Venus', diameterKm: 12104, meanDistanceKm: 108000000 },
      { name: 'Earth', diameterKm: 12756, meanDistanceKm: 149700000 },
      { name: 'Mars', diameterKm: 6792, meanDistanceKm: 227900000 },
      { name: 'Jupiter', diameterKm: 142984, meanDistanceKm: 778000000 },
      { name: 'Saturn', diameterKm: 120536, meanDistanceKm: 1430000000 },
      { name: 'Uranus', diameterKm: 51118, meanDistanceKm: 2900000000 },
      { name: 'Neptune', diameterKm: 49528, meanDistanceKm: 4500000000 },
      { name: 'Pluto', diameterKm: 2377, meanDistanceKm: 5900000000 }
    ],
    dwarfPlanets: [
      'Ceres',
      'Pluto',
...
const UNIVERSE_FACTS = {
  schema: 'simulatte.universeFacts.v1',
  source: 'https://science.nasa.gov/exoplanets/what-is-the-universe/',
  updated: '2026-07-21',
  facts: {
    universeAgeBillionYears: 13.8,
    solarSystemAgeBillionYears: 4.6,
    milkyWay: {
      estimatedStars: 'at least 100 billion',
      nearestMajorGalaxy: {
        name: 'Andromeda',
        distanceMillionLy: 2.5
      }
    },
    galaxies: {
      estimatedObservableCount: 'at least 100 billion'
    },
    observableUniverseSizeLy: 92000000000,
    sizeContext: 'model-dependent and explicit uncertain/expanding-space estimate'
  },
  references: [
...
function writeJsonFile(outPath, payload) {
  ...
  log(`Saved ${payload.schema} to ${outPath}`);
}
...
async function runCity(args) {
  ...
  const outPath = args.find(a => a.startsWith('--out='))?.split('=')[1] || path.join(ROOT, `public/data/simulatte/worlds/${name}-raw-osm.json`);
...
// ---------------------------------------------------------
// TIER 3a: U.S. States Boundaries
// ---------------------------------------------------------
async function runUSStates(args) {
  const outPath = args.find(a => a.startsWith('--out='))?.split('=')[1] || path.join(CACHE_DIR, 'country', 'us-states.geojson');

  log('U.S. States Tier: Downloading state boundary dataset...');
  
  // Public GeoJSON source for U.S. state boundaries.
  ...
  if (!data || !Array.isArray(data.features)) {
    throw new Error('U.S. states payload was malformed (missing features array)');
  }

  fs.writeFileSync(outPath, JSON.stringify(data, null, 2));
  log(`Saved U.S. state boundaries GeoJSON to ${outPath} (features=${data.features.length})`);
}
...
async function runSolarSystem(args) {
  ...
  const factsOutPath = args.find(a => a.startsWith('--facts-out='))?.split('=')[1] || path.join(CACHE_DIR, 'space', 'solar-system-facts.json');
  ...
  for (const [name, id] of Object.entries(bodies)) {
    ...
    const url = `https://ssd.jpl.nasa.gov/api/horizons.api?format=json&COMMAND='${id}'&OBJ_DATA='NO'&MAKE_EPHEM='YES'&EPHEM_TYPE='OBSERVER'&CENTER='500@0'&START_TIME='${startStr}'&STOP_TIME='${stopStr}'&STEP_SIZE='1d'&QUANTITIES='1,20'`;
      ...
      if (!ephemeris.length) {
        throw new Error(`No ephemeris points parsed for ${name}`);
      }
    } catch (err) {
      failedBodies.push(name);
      logError(`Failed to fetch Horizons ephemeris for ${name}`, err);
    }
  }

  if (failedBodies.length > 0) {
    throw new Error(`Incomplete solar-system fetch. Missing bodies: ${failedBodies.join(', ')}`);
  }

  if (Object.keys(results).length === 0) {
    throw new Error('No solar-system ephemerides were written because all body fetches failed');
  }

  writeJsonFile(outPath, results);
  writeJsonFile(factsOutPath, SOLAR_SYSTEM_FACTS);
}
...
async function runStarChart(args) {
  ...
  const factsOutPath = args.find(a => a.startsWith('--facts-out='))?.split('=')[1] || path.join(CACHE_DIR, 'space', 'universe-facts.json');
  ...
  if (stars.length === 0) {
    throw new Error(`No stars parsed for magnitude threshold ${maxMag}`);
  }
  ...
  writeJsonFile(outPath, catalog);
  writeJsonFile(factsOutPath, UNIVERSE_FACTS);
...
async function main() {
  ...
  if (!cmd || cmd === '--help' || cmd === 'help') {
    console.log([
      'Usage:',
      '  node tools/simulatte/fetch-world-tiers.mjs city [--name=nyc-core] [--bounds=south,west,north,east] [--out=path]',
      '  node tools/simulatte/fetch-world-tiers.mjs country [--name=liechtenstein] [--out=path]',
      '  node tools/simulatte/fetch-world-tiers.mjs world [--out=path]',
      '  node tools/simulatte/fetch-world-tiers.mjs us-states [--out=path]',
      '  node tools/simulatte/fetch-world-tiers.mjs solar-system [--start=YYYY-MM-DD] [--stop=YYYY-MM-DD] [--out=path] [--facts-out=path]',
      '  node tools/simulatte/fetch-world-tiers.mjs star-chart [--magnitude=6.0] [--out=path] [--facts-out=path]'
    ].join('\n'));
    process.exit(0);
  }
  ...
  try {
    ...
    } else if (cmd === 'us-states') {
      await runUSStates(subArgs);
